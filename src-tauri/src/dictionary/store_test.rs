...
fn test_complete_crud_workflow() {
    ...
    // Add entry
    let entry = store.add("brb".to_string(), "be right back".to_string()).unwrap();
    ...
    // Update entry
    let updated = store
        .update(entry.id.clone(), "brb".to_string(), "be right back!".to_string())
        .unwrap();
...
fn test_entries_persist_across_reload() {
    ...
    let entry_id = {
        ...
        let entry1 = store.add("brb".to_string(), "be right back".to_string()).unwrap();
        let _entry2 = store.add("api".to_string(), "API".to_string()).unwrap();
        entry1.id
    };
