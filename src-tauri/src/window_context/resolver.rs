...
impl ContextResolver {
    ...
    /// Async version of get_effective_dictionary for use in async contexts.
    ///
    /// This avoids the runtime panic that occurs when calling the sync version
    /// (which uses run_async internally) from within an already-running tokio runtime.
    pub async fn get_effective_dictionary_async(
        &self,
        all_entries: &[DictionaryEntry],
    ) -> Vec<DictionaryEntry> {
        crate::debug!(
            "[ContextResolver] get_effective_dictionary_async called with {} entries",
            all_entries.len()
        );
        ...
        let context_id = match context_id {
            ...
            None => {
                // Get all contexts to find which entries are assigned somewhere
                let assigned_entry_ids: std::collections::HashSet<String> =
                    match self.client.list_window_contexts().await {
                        Ok(contexts) => contexts
                            .iter()
                            .flat_map(|ctx| ctx.dictionary_entry_ids.iter().cloned())
        ...
        // Get the context from Turso
        let context = match self.client.get_window_context(context_id).await {
            Ok(ctx) => ctx,
            Err(e) => {
                crate::warn!(
                    "[ContextResolver] Failed to get context from Turso: {}, returning global dictionary",
    ...
    /// Async version of get_effective_commands for use in async contexts.
    ///
    /// This avoids the runtime panic that occurs when calling the sync version
    /// (which uses run_async internally) from within an already-running tokio runtime.
    pub async fn get_effective_commands_async(
        &self,
        all_commands: &[CommandDefinition],
    ) -> Vec<CommandDefinition> {
        crate::debug!(
            "[ContextResolver] get_effective_commands_async called with {} commands",
            all_commands.len()
        );
        ...
        let context_id = match context_id {
            ...
            None => {
                // Get all contexts to find which commands are assigned somewhere
                let assigned_command_ids: std::collections::HashSet<Uuid> =
                    match self.client.list_window_contexts().await {
                        Ok(contexts) => contexts
                            .iter()
                            .flat_map(|ctx| ctx.command_ids.iter().cloned())
